...
class Geth extends HttpProvider implements Provider {
	...
	private function geth_request($method, $params=array())
	{
		try 
		{
	...
	function personal_unlockAccount(string $address, string $address, int $duration = 0)
	{
		return $this->geth_request(__FUNCTION__ , [$address, $address, $duration] );
	}
